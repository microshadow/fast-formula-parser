...
const EngineeringFunctions = {
    ...
    BIN2HEX: (number, places) => {
        ...
        if (numberStr.length === 10 && numberStr.substring(0, 1) === '1') {
            return (parseInt(numberStr.substring(1), 2) + 1099511627264).toString(16)
        }
    ...
    BIN2OCT: (number, places) => {
        ...
        places = Math.floor(places);
        return (result.length <= places) ? TextFunctions.REPT(0,places - result.length) + result : FormulaError.NUM;
    },
